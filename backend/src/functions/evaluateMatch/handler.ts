...
export async function handler(event: APIGatewayProxyEventV2) {
  try {
    ...
    if (!jobId) return jsonResponse(400, { message: "Missing required field: jobId" });
    if (!cvKey) return jsonResponse(400, { message: "Missing required field: cvKey" });
    ...
    const job = jobResponse.Item as Job;
